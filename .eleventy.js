...
module.exports = function (config) {
  ...
  config.addFilter("abstract", function (input) {
    ...
    const y = $("p:first-of-type ~ :not(h2, h3, h4, ol, li, ul)")
      .eq(0)
      .toString();
    return x + y;
  });
  ...
  config.addFilter("replaceStart", function (input, ...args) {
    if (input.startsWith(args[0])) {
      return input.replace(...args);
    }
    return input;
  });
  ...
  config.addCollection("categories", function (collection) {
    ...
    categories.forEach((el) => {
      cats[el] = Array.from(collection.items)
        ...
        .filter(
          (item) => item.data.categories && item.data.categories.includes(el)
        )
        .sort((a, b) => {
